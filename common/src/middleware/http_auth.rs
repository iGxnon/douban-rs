...
mod test {
    use crate::middleware::http_auth::{AuthBackend, HttpAuthLayer};
    ...
    impl AuthBackend<UserId> for BlindBackend {
        ...
        fn auth_basic_async(
            ...
            Box::pin(async { Ok((UserIdU64(1), None)) })
        }
        ...
        fn auth_bearer(
            ...
            Box::pin(async { Ok((UserIdU64(2), Some("new_token".to_string()))) })
        }
    }

    #[tokio::test]
    async fn test_cookie_service() {
        tracing_subscriber::fmt::init();
        let mut app = ServiceBuilder::new()
            .layer(HttpAuthLayer::without_cookie(BlindBackend, true))
            .service_fn(|_req: Request<Body>| async move {
                Result::<Response<Body>, String>::Ok(Response::new(Body::empty()))
            });
        let res: Response<Body> = app
            .ready()
            .await
            .unwrap()
            .call(
    ...
    #[tokio::test]
    async fn test_axum_auth() {
        Router::<()>::new()
            .route("/foo", get(|| async { (StatusCode::OK, "hi") }))
            .layer(HttpAuthLayer::new(BlindBackend, false, true, 172800));
    }
}
